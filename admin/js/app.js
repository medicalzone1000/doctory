      ...
      <div class="form-grid">
        ...
        <label class="field"><span>التخصص</span>
          <select class="input" name="specialty" id="docSpecialtySelect">
            ${specs.map((s) => `<option value="${escapeHtml(s.name)}" data-icon="${escapeHtml(s.icon)}" ${doc.specialty === s.name ? "selected" : ""}>${escapeHtml(s.name)}</option>`).join("")}
          </select>
        </label>
...
زمالة الكلية الملكية للأطباء - لندن">${escapeHtml((doc.qualifications || []).join("\n"))}</textarea>
      ...
      <div class="modal is-open">
        ...
        <div class="modal__dialog">
          ...
          <form id="specForm" class="form-grid">
            ...
            <label class="field">
              <span>الأيقونة</span>
              <select class="input" name="icon" id="specIconSelect">
                ${Object.keys((window.DoctoryIcons && window.DoctoryIcons.GLYPHS) || { stethoscope: 1 })
                  .map((key) => `<option value="${key}" ${((s.icon || "stethoscope") === key) ? "selected" : ""}>${key}</option>`)
                  .join("")}
              </select>
            </label>
            <div class="field"><span>معاينة</span><div id="specIconPreview" style="width:40px;height:40px;display:flex;align-items:center;justify-content:center;color:var(--accent-teal,#1E5C4A);border:1px solid var(--line,#e5e5e5);border-radius:8px;">${window.DoctoryIcons ? window.DoctoryIcons.svg(s.icon || "stethoscope", { size: "22px" }) : ""}</div></div>
          </form>
          <div class="modal__foot">
            <button class="btn btn--ghost" data-close-modal>إلغاء</button>
      ...
      return `
        <div class="section-row" data-sec-id="${escapeHtml(sec.id)}" style="background:white;border:1.5px solid #e5e7eb;border-radius:14px;padding:18px 20px;margin-bottom:10px;display:flex;align-items:center;gap:16px;cursor:grab;transition:box-shadow 0.2s;">
          ...
          <div style="flex:1;min-width:0;">
            ...
            ${sec.eyebrow ? `<div style="font-size:12px;color:#1E5C4A;margin-top:3px;">الشارة: ${escapeHtml(sec.eyebrow)}</div>` : ''}
          </div>
          <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
            <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;">
              <input type="checkbox" class="sec-visible-toggle" data-idx="${idx}" ${sec.visible !== false ? 'checked' : ''} style="width:16px;height:16px;accent-color:#1E5C4A;">
              ظاهر
            </label>
  ...
  function bindHomeSectionsEvents() {
    ...
    document.getElementById('addTestimBtn')?.addEventListener('click', () => {
      cfg.testimonials.push({ id: Date.now(), name: '', label: '', text: '', rating: 5, avatarColor: '#1E5C4A' });
      setDirty();
      renderSection();
    });
  }
  ...
  function renderDesign() {
    ...
    const themeFields = [
      ...
      ["accentTeal", "لون التمييز (أخضر طبي)"],
      ["lilac300", "درجة وسطى من لون التمييز"],
      ["lilac100", "خلفية فاتحة بلون التمييز"],
      ["paper", "خلفية الصفحة"],
      ["textMuted", "نص ثانوي"],
    ];
  ...
  function toColorInput(val) {
    if (!val || val.startsWith("rgba")) return "#101B22";
  ...
  function bindSectionEvents() {
    ...
    $("#addSpecBtn")?.addEventListener("click", () => {
      state.editingSpecialty = { name: "", icon: "stethoscope" };
      renderSection();
    });
    ...
    $("#specIconSelect")?.addEventListener("change", (e) => {
      const preview = $("#specIconPreview");
      if (preview && window.DoctoryIcons) preview.innerHTML = window.DoctoryIcons.svg(e.target.value, { size: "22px" });
    });
  ...
  function saveDoctorForm() {
    ...
    const doc = {
      ...
      specialtyIcon: specOption?.dataset.icon || "stethoscope",
