...
document.addEventListener("DOMContentLoaded", () => {
  const boot = () => {
    ...
    if (!Number.isFinite(doctorId)) {
      document.getElementById("profileCard").innerHTML = `
        <div class="empty-state" style="padding:60px 20px; text-align:center;">
          <div class="empty-state__icon">${DoctoryIcons.svg("warning",{size:"1em"})}</div>
    ...
    if (!doctor) {
      document.getElementById("profileCard").innerHTML = `
        <div class="empty-state" style="padding:60px 20px; text-align:center;">
          <div class="empty-state__icon">${DoctoryIcons.svg("help-circle",{size:"1em"})}</div>
...
function renderProfile(doctor) {
  ...
  const badges = [];
  if (doctor.verified) badges.push(`<span class="profile-card__badge profile-card__badge--verified">${DoctoryIcons.svg("check",{size:"0.9em"})} موثق</span>`);
  ...
  container.innerHTML = `
    <div class="profile-card__grid">
      ...
      <div class="profile-card__info">
        ...
        <div class="profile-card__specialty">${DoctoryIcons.svg(doctor.specialtyIcon)} ${doctor.specialty}</div>
        ...
        <div class="profile-card__meta">
          <span class="profile-card__pill profile-card__pill--exp">${DoctoryIcons.svg("person",{size:"0.9em"})} ${doctor.experience} سنة خبرة</span>
          <span class="profile-card__pill profile-card__pill--price">${DoctoryIcons.svg("price",{size:"0.9em"})} ${doctor.price} جنيه</span>
          <span class="profile-card__pill profile-card__pill--location">${DoctoryIcons.svg("pin",{size:"0.9em"})} ${doctor.area}، ${doctor.governorate}</span>
        </div>
    ...
    <div class="profile-details">
      ...
      <div class="profile-details__section">
        ...
        <div class="map-placeholder">
          <span>${DoctoryIcons.svg("pin",{size:"0.9em"})} خريطة الموقع (تجريبي)</span>
        </div>
      </div>
    </div>
  `;
  ...
  related.forEach(d => {
    ...
    card.innerHTML = `
      <div class="related-doctor-card__top">
        <div class="related-doctor-card__avatar">${d.initials}</div>
        <div class="related-doctor-card__info">
          <div class="related-doctor-card__name">${d.name}</div>
          <div class="related-doctor-card__specialty">${DoctoryIcons.svg(d.specialtyIcon)} ${d.specialty}</div>
