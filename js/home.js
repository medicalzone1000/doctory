...
function buildSpecialtiesHomeGrid() {
  ...
  MEDICAL_SPECIALTIES.slice(0, 14).forEach((spec, i) => {
    ...
    a.innerHTML = `<div class="spec-card__icon">${DoctoryIcons.svg(spec.icon)}</div><span class="spec-card__name">${spec.name}</span>`;
    grid.appendChild(a);
  });
}
...
function buildFeaturedDoctors() {
  ...
  featured.forEach((doc, i) => {
    ...
    const avatarHTML = avatarImg
      ...
      : `<div class="doctor-card__avatar doctor-card__avatar--placeholder">${doc.initials||doc.name.slice(0,2)}</div>`;
  ...
  grid.innerHTML = list.map((t, i) => `
    <div class="testimonial-card reveal" style="transition-delay:${i*80}ms">
      ...
      <div class="testimonial-card__author">
        <div class="testimonial-card__avatar">${t.name.slice(0,1)}</div>
        <div>
    ...
    <div class="container app-cta__inner">
      ...
      <div class="app-cta__mockup reveal" aria-hidden="true">
        <svg viewBox="0 0 200 360" fill="none" class="phone-svg">
          ...
          <rect x="24" y="36" width="152" height="298" rx="18" fill="#1E2E36" opacity=".5"/>
          <rect x="24" y="36" width="152" height="44" rx="18" fill="#101B22" opacity=".9"/>
          <circle cx="50" cy="58" r="12" fill="#1E5C4A" opacity=".6"/>
          ...
          <rect x="24" y="309" width="152" height="25" rx="18" fill="#101B22" opacity=".9"/>
          <circle cx="54" cy="318" r="8" fill="white" opacity=".6"/>
          <circle cx="100" cy="318" r="8" fill="#1E5C4A" opacity=".8"/>
