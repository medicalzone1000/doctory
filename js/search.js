...
function readURLParams() {
  ...
  if (state.specialty) {
    ...
    if (specInput) {
      ...
      specInput.value = found ? found.name : state.specialty;
    }
  }
...
function initSearchDropdowns() {
  ...
  setupDropdownField({
    ...
    getItems: () => MEDICAL_SPECIALTIES.map((s) => s.name),
    onSelect: (value) => {
      state.specialty = value.trim();
    },
  });
...
function initBurgerMenu() {
  ...
  if (grid) {
    MEDICAL_SPECIALTIES.forEach((spec) => {
      ...
      a.innerHTML = `<span class="icon">${DoctoryIcons.svg(spec.icon)}</span><span>${spec.name}</span>`;
      grid.appendChild(a);
    });
  }
...
function buildDoctorCard(doc) {
  ...
  const avatarHTML = avatarImg
    ...
    : `<div class="doctor-card__avatar-placeholder">
  ...
  card.innerHTML = `
    ...
    <div class="doctor-card__top">
      <div class="doctor-card__avatar-wrap">
        ${avatarHTML}
        ${doc.verified ? `<span class="doctor-card__badge doctor-card__badge--verified">${DoctoryIcons.svg("check",{size:"0.9em"})} موثق</span>` : ""}
      </div>
      <div class="doctor-card__info">
        <div class="doctor-card__name">${doc.name}</div>
        <div class="doctor-card__specialty">${DoctoryIcons.svg(doc.specialtyIcon)} ${doc.specialty}</div>
...
function renderActiveTags() {
  ...
  if (state.governorate) {
    tags.push({ label: `${DoctoryIcons.svg("pin",{size:"0.9em"})} ${state.governorate}`, remove: () => {
      state.governorate = "";
      state.area = "";
      currentAreaList = [];
  ...
  if (state.area) {
    tags.push({ label: `${DoctoryIcons.svg("pin",{size:"0.9em"})} ${state.area}`, remove: () => { state.area = ""; const el = document.getElementById("area"); if (el) el.value = ""; } });
  }
  if (state.specialty) {
    tags.push({ label: `${DoctoryIcons.svg(state.specialty,{size:"0.9em"})} ${state.specialty}`, remove: () => { state.specialty = ""; const el = document.getElementById("specialty"); if (el) el.value = ""; } });
  }
  if (state.doctor) {
    tags.push({ label: `${DoctoryIcons.svg("person",{size:"0.9em"})} ${state.doctor}`, remove: () => { state.doctor = ""; const el = document.getElementById("doctor"); if (el) el.value = ""; } });
  }
  if (state.gender) {
  ...
  if (state.priceRange !== "0-99999") {
    ...
    tags.push({ label: `${DoctoryIcons.svg("price",{size:"0.9em"})} ${min}–${max === "99999" ? "∞" : max} جنيه`, remove: () => { state.priceRange = "0-99999"; resetChipGroup("priceFilter"); } });
  }
